...
exports.getTourStats = catchAsync(async (req, res, next) => {
        const stats = await Tour.aggregate([
            { $match: { ratingsAverage: { $gte: 4.5 } } // to select document 
            },
            {
                $group: {
                    _id: { $toUpper: '$difficulty'}, // to specify what group by avgPrice
                    numTour: { $sum: 1 },
...
exports.getMonthPlan = catchAsync(async (req, res, next) => {
        ...
        const plan = await Tour.aggregate([
            ...
            {
                $group: {
                    _id: { $month: '$startDates' }, // select month
                    numTourStarts: { $sum: 1 },
                    tours: { $push: '$name'}       // to push name of tour in array 
                }
            },
            {
                $addFields: { month: '$_id'}
...
exports.getToursWithin = catchAsync(async (req,res,next) => {
    ...
    if(!lat || !lng){
        return next(new AppError('please provid latitutr and longitude in lat lng form',400));
    }
